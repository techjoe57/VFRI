		...
		<section
			...
			<div className="rf-section-py relative w-full px-6 md:px-12">
				...
				<div ref={introRef} className="rf-fade-up rf-delay-0">
					...
					<p className="mt-6 font-sans text-2xl font-semibold leading-snug text-ink md:mt-8 xl:text-3xl min-[2560px]:text-4xl">
				...
				<div className="mt-8 border-t border-ink/10 divide-y divide-ink/10">
					{OUTCOMES.map((item, index) => (
						<div
							...
							<span
								className={`mt-0.5 flex-shrink-0 font-sans text-base font-semibold tracking-[0.1em] ${item.accent} xl:text-lg min-[2560px]:text-xl`}>
								0{index + 1}
							</span>
							<p className="text-lg leading-relaxed text-ink xl:text-xl min-[2560px]:text-2xl">
								{item.text}
							</p>
						</div>
					))}
